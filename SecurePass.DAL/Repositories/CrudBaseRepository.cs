...
public abstract class CrudBaseRepository<TEntity> : ICrudBaseRepository<TEntity> where TEntity : BaseEntity
{
    ...
    public virtual async Task<TEntity> GetById(int entityId, string? include)
    {
    ...
    public virtual async Task<IEnumerable<TEntity>> GetList(string? include = null, Expression<Func<TEntity, bool>>? predicate = null)
    {
        IQueryable<TEntity> query = _entities;
